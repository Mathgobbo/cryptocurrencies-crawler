import { CRYPTOCOMPARE_URL } from "../constants.mjs";
import { sleep, writeJsonToFile } from "../utils.mjs";

export const extractFromCryptocompare = async (browser) => {
  // Timer start
  console.time("cryptocompare");

  const page = await browser.newPage();
  await page.goto(CRYPTOCOMPARE_URL);
  ...
  for (let tokensTablePage = 1; tokensTablePage < 2; tokensTablePage++) {
    ...
    for (let index = 0; index < savedLinks.length; index++) {
      ...
      await page.goto(CRYPTOCOMPARE_URL + link);
      ...
      tokensDetails.push({ ...tokenDetails, link: CRYPTOCOMPARE_URL + link });
    ...
    // Writing data...
    await writeJsonToFile(tokensDetails, "cryptocompare-dataset.json");

    // Timer end
    console.timeEnd("cryptocompare");
  }
};
